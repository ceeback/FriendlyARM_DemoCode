...
static void _WrL3Addr(UCHAR data);
static void _WrL3Data(UCHAR data,int halt);
static void Init1341(char mode);
static void AdjVolume(USHORT volume);
static void PlayPause(UCHAR mode);
...
static ULONG PlayTotBlks, RecTotBlks;

static ULONG save_MPLLCON;
...
static USHORT PlayVolume = 0xffff;
//static UCHAR inner_buf[32*1024];
...
static void SetSysFclk( ULONG MPLL )
{
	ULONG mdiv, pdiv, sdiv, val;
    ULONG mclk;
...
MMRESULT waveOutPrepareHeader(HWAVEOUT hwo,	LPWAVEHDR pwh, ULONG cbwh)
{
	if((PlayStatus.handle!=hwo)||(PlayStatus.Status!=DEVICE_PLAY))
		return MMSYSERR_INVALHANDLE;
...
MMRESULT waveOutUnprepareHeader(HWAVEOUT hwo, LPWAVEHDR pwh, ULONG cbwh)
{
	if((PlayStatus.handle!=hwo)||(PlayStatus.Status!=DEVICE_PLAY))
		return MMSYSERR_INVALHANDLE;
...
MMRESULT waveOutWrite(HWAVEOUT hwo,	LPWAVEHDR pwh, ULONG cbwh)
{
	if((PlayStatus.handle!=hwo)||(PlayStatus.Status!=DEVICE_PLAY))
		return MMSYSERR_INVALHANDLE;
...
MMRESULT waveOutSetVolume(HWAVEOUT hwo,	ULONG dwVolume)
{
	if((PlayStatus.handle!=hwo)||(PlayStatus.Status!=DEVICE_PLAY))
		return MMSYSERR_INVALHANDLE;
...
{
//	WrUTXH0('@');
	SetDMARun(PlayStatus.DevReq|DMA_START, (ULONG)lpPlayCurBlk->lpData, (ULONG)IISFIFO, lpPlayCurBlk->dwBufferLength/2);
}
...
{
//	WrUTXH0('.');
	SetDMARun(RecStatus.DevReq|DMA_START, (ULONG)IISFIFO, (ULONG)lpRecCurBlk->lpData, lpRecCurBlk->dwBufferLength/2);
}
...
)
{
	...
	if((RecStatus.DevReq&0xff)==0x12)
	{
		pISR_DMA1 = (ULONG)RecDMADone;
		EnableIrq(BIT_DMA1);
	}
	if((RecStatus.DevReq&0xff)==0x21)
	{
		pISR_DMA2 = (ULONG)RecDMADone;
		EnableIrq(BIT_DMA2);
	}
...
MMRESULT waveInPrepareHeader(HWAVEIN hwi, LPWAVEHDR pwh, ULONG cbwh)
{
	if((RecStatus.handle!=hwi)||(RecStatus.Status!=DEVICE_REC))
		return MMSYSERR_INVALHANDLE;
...
MMRESULT waveInUnprepareHeader(HWAVEIN hwi, LPWAVEHDR pwh, ULONG cbwh)
{
	if((RecStatus.handle!=hwi)||(RecStatus.Status!=DEVICE_REC))
		return MMSYSERR_INVALHANDLE;
...
MMRESULT waveInAddBuffer(HWAVEIN hwi, LPWAVEHDR pwh, ULONG cbwh)
{
	if((RecStatus.handle!=hwi)||(RecStatus.Status!=DEVICE_REC))
		return MMSYSERR_INVALHANDLE;
