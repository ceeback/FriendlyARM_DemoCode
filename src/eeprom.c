...
static int iic_wr(UCHAR data, UCHAR cnt)
{
	int i, c;
...
static int seek_iic_device(UCHAR slvAddr)
{
	int i;
...
static int write_eeprom(UCHAR slvAddr, UCHAR addr, UCHAR data)
{
	int i;
	UCHAR ch[3];
...
//8 bits register address only
static int read_eeprom(UCHAR slvAddr, UCHAR addr, UCHAR *pBuf)
{
	int i;
	UCHAR ch[2];
