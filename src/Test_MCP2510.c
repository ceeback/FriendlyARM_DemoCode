...
void MCP2510_IO_CS_Init( void )
{
   ...
   { USHORT k=0; for( ; k <= DELAY_TIME; k++ ) ;  }  //延时至少300ns
...
****************************************************************************/
void MCP2510_Write( UCHAR address, UCHAR value)
{
	MCP2510_RW_Start() ;
...
【功能说明】修改指定地址寄存器的某些位
****************************************************************************/
void MCP2510_WriteBits( UCHAR address, UCHAR data, UCHAR mask )
{
	MCP2510_RW_Start() ;
...
****************************************************************************/
UCHAR MCP2510_Read( UCHAR address )
{
	UCHAR result;
...
【功能说明】序列读取MCP2510数据
****************************************************************************/
void MCP2510_SRead( UCHAR address, UCHAR* pdata, UCHAR nlength )
{
	int i;
...
【功能说明】序列写入MCP2510数据
****************************************************************************/
void MCP2510_Swrite( UCHAR address, UCHAR* pdata, UCHAR nlength)
{
	int i;
	MCP2510_RW_Start() ;

	Spi_Write(MCP2510INSTR_WRITE);
	Spi_Write((UCHAR)address);

	for (i=0; i < nlength; i++)
	{
		Spi_Write( (UCHAR)*pdata );
		//if( MCP2510_DEBUG )    Uart_Printf( "0x%x\n", (UCHAR)*pdata ) ;
		pdata++;
	}
	MCP2510_CS_H ;
...
FALSE，表示非扩展ID(11位)
****************************************************************************/
int MCP2510_Read_Can_ID( UCHAR address, ULONG* can_id)
{
	ULONG tbufdata;
	UCHAR* p=(UCHAR*)&tbufdata;
...
*		FALSE，表示非扩展总线						*
\***********************************************************/
int MCP2510_Read_Can(UCHAR nbuffer, int* rxRTR, ULONG* can_id, UCHAR* data , UCHAR* dlc)
{

	UCHAR mcp_addr = (nbuffer<<4) + 0x31, ctrl;
	int IsExt;
...
*		FALSE，表示非扩展总线						*
\***********************************************************/
void MCP2510_Write_Can( UCHAR nbuffer, int ext, ULONG can_id, int rxRTR, UCHAR* data,UCHAR dlc )
{
	UCHAR mcp_addr = (nbuffer<<4) + 0x31;
...
*			IsExt表示是否为扩展ID	*
\*******************************************/
void MCP2510_Write_Can_ID(UCHAR address, ULONG can_id, int IsExt)
{
	ULONG tbufdata;
	...
	MCP2510_Swrite(address, (UCHAR*)&tbufdata, 4);
}
...
\***********************************************************************************/
void Can_Write(ULONG id, UCHAR *pdata, UCHAR dlc, int IsExt, int rxRTR)
{
	UCHAR err ;
...
【功能说明】
****************************************************************************/
int Can_Read(int n, ULONG* id, UCHAR *pdata,  UCHAR*dlc, int* rxRTR, int *isExt)
{
	UCHAR byte;
	...
	if(n==0)
	{
		if(byte & RX0INT)
		{
			*isExt=MCP2510_Read_Can(n+3, rxRTR, id, pdata, dlc);
			MCP2510_WriteBits(CANINTF, (UCHAR)(~(RX0INT)), RX0INT); // Clear interrupt
			return TRUE ;
		}
	...
	else if(n ==1 )
	{
		if(byte & RX1INT)
		{
			*isExt=MCP2510_Read_Can(n+4, rxRTR, id, pdata, dlc);
			MCP2510_WriteBits(CANINTF, (UCHAR)(~(RX1INT)), RX1INT); // Clear interrupt
			return TRUE ;
		}
...
void Test_MCP2510(void)
{
	...
	UCHAR data_write[8]={1,2,3,4,5,6,7,8};
	UCHAR data_read[8] ;
