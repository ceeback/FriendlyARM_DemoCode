...
#else
static ULONG WaitNFBusy(void)	// R/B 未接好?
{
	UCHAR stat;
...
static ULONG EraseBlock(ULONG addr)
{
	UCHAR stat;
...
//addr = page address
static void ReadPage(ULONG addr, UCHAR *buf)
{
	USHORT i;
...
static ULONG WritePage(ULONG addr, UCHAR *buf)
{
	ULONG i, mecc;
	UCHAR stat, tmp[7];
...
static void MarkBadBlk(ULONG addr)
{
	addr &= ~0x1f;
...
static int CheckBadBlk(ULONG addr)
{
	UCHAR dat;
...
*/
static ULONG StartPage, BlockCnt;
...
//		return;
	...
	for(i=0; size>0; )	{
		...
		if(WritePage(i+StartPage, (UCHAR *)ram_addr)) {
			ram_addr -= (i&0x1f)<<9;
			size += (i&0x1f)<<9;
			i &= ~0x1f;
			NandPart[nf_part].size -= 32<<9;	//partition available size - 1 block size
...
#define FLAG_RDPROMPT	8
	    ...
	    UCHAR sounddefault;			/* 40 */
	    UCHAR adfsdrives;			/* 41 */
	    UCHAR bytes_per_char_h;		/* 42 */
	    UCHAR bytes_per_char_v;		/* 43 */
...
extern void  call_linux(ULONG a0, ULONG a1, ULONG a2);
...
//	SetSysFclk(FCLK_200M);		//start kernel, use 200M
	SET_IF();
	goto_start = (void (*)(ULONG, ULONG))a2;
	(*goto_start)(a0, a1);
}

static void LoadRun(void)
{
	ULONG i, ram_addr, buf = boot_params.run_addr.val;
	struct param_struct *params = (struct param_struct *)0x30000100;
	int size;

	memset(params, 0, sizeof(struct param_struct));
	/*{
		ULONG *pD = (ULONG *)0x30000100;
	...
	for(i=0; size>0; ) {
		...
		ReadPage((i+StartPage), (UCHAR *)ram_addr);
		i++;
		size -= 512;
		ram_addr += 512;
	}
