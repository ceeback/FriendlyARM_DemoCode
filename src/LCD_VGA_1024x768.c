...
//TFT 1024x768
#define LCD_XSIZE_VGA_1024x768 	(1024)
#define LCD_YSIZE_TFT_800x480 	(768)

//TFT 1024x768
#define SCR_XSIZE_VGA_1024x768 	(1024)
#define SCR_YSIZE_VGA_1024x768 	(768)

//TFT800x480
#define HOZVAL_VGA_1024x768	(LCD_XSIZE_VGA_1024x768-1)
#define LINEVAL_VGA_1024x768	(LCD_YSIZE_TFT_800x480-1)

//以下参数只针对VGA
//# 640x400 @ 70 Hz, 31.5 kHz hsync
//Modeline "640x400" 25.175 640 664 760 800 400 409 411 450
#define HFPD_1024x768		(1)		//水平同步信号的前肩
#define HSPW_1024x768		(1)		//水平同步信号的脉宽
#define HBPD_1024x768		(17)		//水平同步信号的后肩

#define VFPD_1024x768		(15)		//垂直同步信号的前肩
#define VSPW_1024x768		(199)		//垂直同步信号的脉宽
#define VBPD_1024x768		(15)	//垂直同步信号的后肩

extern UCHAR VGA_1024x768_DEMO[];	//宽1024，高768

volatile static unsigned short LCD_BUFER[SCR_YSIZE_VGA_1024x768][SCR_XSIZE_VGA_1024x768];
...
{
	rLCDCON1=(CLKVAL_VGA_1024x768<<8)|(1<<7)|(3<<5)|(12<<1)|0;
    	// TFT LCD panel,16bpp TFT,ENVID=off
	rLCDCON2=(VBPD_1024x768<<24)|(LINEVAL_VGA_1024x768<<14)|(VFPD_1024x768<<6)|(VSPW_1024x768);
	rLCDCON3=(HBPD_1024x768<<19)|(HOZVAL_VGA_1024x768<<8)|(HFPD_1024x768);
	rLCDCON4=(MVAL<<8)|(HSPW_1024x768);
	...
	rLCDSADDR2=M5D( ((ULONG)LCD_BUFER+(SCR_XSIZE_VGA_1024x768*LCD_YSIZE_TFT_800x480*2))>>1 );
	rLCDSADDR3=(((SCR_XSIZE_VGA_1024x768-LCD_XSIZE_VGA_1024x768)/1)<<11)|(LCD_XSIZE_VGA_1024x768/1);
...
static void Lcd_MoveViewPort(int vx,int vy)
{
    ...
    SET_IF();
	#if (LCD_XSIZE_VGA_1024x768<32)
    	    while((rLCDCON1>>18)<=1); // if x<32
	#else
    	    while((rLCDCON1>>18)==0); // if x>32
	#endif
        addr=(ULONG)LCD_BUFER+(vx*2)+vy*(SCR_XSIZE_VGA_1024x768*2);
	rLCDSADDR1= ( (addr>>22)<<21 ) | M5D(addr>>1);
	rLCDSADDR2= M5D(((addr+(SCR_XSIZE_VGA_1024x768*LCD_YSIZE_TFT_800x480*2))>>1));
	CLR_IF();
}
...
static void MoveViewPort(void)
{
    ...
    while(1)
    {
    	...
    	case '6':
                if(vx<=(SCR_XSIZE_VGA_1024x768-LCD_XSIZE_VGA_1024x768-vd))vx+=vd;
   	    break;

    	case '2':
                if(vy<=(SCR_YSIZE_VGA_1024x768-LCD_YSIZE_TFT_800x480-vd))vy+=vd;
   	    break;
...
{
    if(x<SCR_XSIZE_VGA_1024x768 && y<SCR_YSIZE_VGA_1024x768)
		LCD_BUFER[(y)][(x)] = c;
}
...
static void Lcd_ClearScr( USHORT c)
{
    ...
    for( y = 0 ; y < SCR_YSIZE_VGA_1024x768 ; y++ )
    {
    	for( x = 0 ; x < SCR_XSIZE_VGA_1024x768 ; x++ )
    	{
			LCD_BUFER[y][x] = c ;
    	}
    }
...
static void Paint_Bmp(int x0,int y0,int h,int l,UCHAR bmp[])
{
    ...
    for( y = 0 ; y < l ; y++ )
    {
    	for( x = 0 ; x < h ; x++ )
    	{
			...
			if ( ( (x0+x) < SCR_XSIZE_VGA_1024x768) && ( (y0+y) < SCR_YSIZE_VGA_1024x768) )
...
/**************************************************************
**************************************************************/
void Lcd_VGA_1024x768_Init(void)
{
...
*/
	...
	#define LCD_BLANK		30
	#define C_UP		( LCD_XSIZE_VGA_1024x768 - LCD_BLANK*2 )
	#define C_RIGHT		( LCD_XSIZE_VGA_1024x768 - LCD_BLANK*2 )
	#define V_BLACK		( ( LCD_YSIZE_TFT_800x480 - LCD_BLANK*4 ) / 6 )
	Glib_FilledRectangle( LCD_BLANK, LCD_BLANK, ( LCD_XSIZE_VGA_1024x768 - LCD_BLANK ), ( LCD_YSIZE_TFT_800x480 - LCD_BLANK ),0x0000);		//fill a Rectangle with some color
...
/*
    ...
    Uart_Getch() ;		//wait uart input
*/
    Paint_Bmp(0,0,1024,768, VGA_1024x768_DEMO);		//paint a bmp
   	Uart_Printf( "LCD paint a bmp, please look! press any key to continue! \n" );
    Uart_Getch() ;		//wait uart input
